...
const CertificationCard: React.FC<CertificationCardProps> = ({badge}: CertificationCardProps) => {
    const skillsNames = badge.badge_template.skills.map(skillEntity => skillEntity.name)
    ...
    return (
        <li className="overflow-hidden lg:p-6 text-center flex flex-col gap-2">
            <Image width={200} height={200} className="w-full max-w-[200px] mx-auto p-4"
                   src={transformCredlyUrl(imgUrl)} alt={`${title} Badge`}/>
